...
class Lamb_Data:
    ...
    def pad_locations(self, latlongs, truncation=True):
        latlongs = [[latlong[0]/90, latlong[1]/180] for latlong in latlongs] # normalise for question latlong
        if truncation and len(latlongs) > self.opts.max_locations:
            mask = [True] * self.opts.max_locations
            return latlongs[:self.opts.max_locations], mask
        diff = self.opts.max_locations - len(latlongs)
    ...
    def batch_process_candidates(self, batch, training=False):
    ...
    def prepare_forward(self, batch, training=True, device='cpu'):
        ...
        candidate_inputs = self.batch_process_candidates(batch, training=training)
        ...
        need_latlong = self.opts.location or self.opts.distance
        # seperate inputs
        ...
        c_latlongs = candidate_inputs['latlongs'] if need_latlong else None
        c_emb = candidate_inputs['embeds'] if not training else None
        question_inputs.pop('latlongs',None)
        question_inputs.pop('latlong_mask',None)
        candidate_inputs.pop('latlongs',None)
        candidate_inputs.pop('embeds',None)

        batch = ({"question_inputs": question_inputs,
                  "question_latlongs": q_latlongs,
                  "question_latlong_mask": q_latlong_mask,
                  "candidate_inputs":candidate_inputs,
                  "candidate_latlongs":c_latlongs,
                  "candidate_entity_vectors":c_emb})
        return batch
